...
import { formatColones, formatFechaCorta, getSigno, getEdad, getIniciales } from '../../utils';
...
export function FichaCliente({ cliente, citas, notas, onAgregarNota, onActualizarTags, onVolver }: Props) {
  ...
  return (
    <div>
      <button onClick={onVolver} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 12, cursor: 'pointer', letterSpacing: '0.05em', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6 }}>
        ← Volver
      </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: '1.5rem' }}>
        {[
          { label: 'Teléfono', value: cliente.telefono, sub: '' },
          { label: 'Nacimiento', value: formatFechaCorta(cliente.fechaNacimiento), sub: `${getSigno(cliente.fechaNacimiento)} · ${getEdad(cliente.fechaNacimiento)} años` },
          { label: 'Modalidad', value: cliente.modalidadUsual === 'presencial' ? 'Presencial' : 'Virtual', sub: '' },
        ].map(d => (
          <div key={d.label}>
            <span style={lbl}>{d.label}</span>
            <p style={{ fontSize: 14, color: 'var(--text-primary)' }}>{d.value}</p>
            {d.sub && <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>{d.sub}</p>}
          </div>
        ))}
      </div>
        ...
        <div key={campo} style={{ marginBottom: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            {(cliente[campo] ?? []).map(tag => (
              <span key={tag} className={`tag ${cls}`} onClick={() => onActualizarTags(campo, (cliente[campo] ?? []).filter(t => t !== tag))} title="Click para eliminar">
            ...
            {editandoTag === campo ? (
              <div style={{ display: 'flex', gap: 6 }}>
                <input autoFocus value={nuevoTag[campo]}
                  ...
                  placeholder="Escribí y Enter" style={{ fontSize: 12, padding: '3px 8px', width: 140 }} />
              </div>
            ) : (
      ...
      {notas.map(nota => {
        ...
        return (
          <div key={nota.id} className="nota-item">
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5, letterSpacing: '0.04em' }}>
      ...
      <div style={{ marginTop: '1rem' }}>
        ...
        <button className="btn-primary" onClick={() => { if (!nuevaNota.trim()) return; onAgregarNota(citaSeleccionada, nuevaNota); setNuevaNota(''); }} disabled={!nuevaNota.trim()}>
          Guardar nota
        </button>
      </div>
